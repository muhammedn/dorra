...
include 'init.php';
  ...
  if($_GET['edit']){
   ...
   $getInfo = $con->prepare("SELECT * FROM project_highlight WHERE ID = ?");
   ...
   $project = $getInfo->fetch();
    ...
    if(isset($_POST['edit'])){
         ...
         $stmt = $con->prepare("UPDATE project_highlight SET p_name = ? , p_desc = ? , location = ?, type = ? , generalp = ?, highlight = ?, id_cat = ? ,p_image = ? , p_image2 = ? , p_image3 = ? ,p_image4 = ?  WHERE ID = ? ");
